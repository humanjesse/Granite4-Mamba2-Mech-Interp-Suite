...
- **Circuit Discovery** via path patching to trace how information flows between layers — goes beyond single-layer importance to map the connections that matter. Builds a full path matrix showing recovery for every source→target layer pair, identifies critical paths in the hybrid architecture (Mamba→Transformer handoffs), extracts attention head importance within Transformer layers, and automatically constructs a minimal circuit graph above a configurable threshold. Visualized as a path matrix heatmap, layer importance bar chart, component importance grid, and circuit diagram with Mamba/Transformer nodes and directed edges
...
8. **Circuit Discovery**: Path patching engine that traces information flow between layers. Standard activation patching tells you *which layers matter*; path patching tells you *which connections between layers matter*. For each source→target pair, the corrupted forward pass is modified at the target layer's input by subtracting the corrupted source's residual contribution and adding the clean source's: `residual_at_target = corrupted - source_corrupted_contrib + source_clean_contrib`. This isolates the specific pathway. The engine runs in three phases: (1) layer sweep for node importance, (2) O(n²/2) path patches for the upper-triangular connection matrix, (3) optional component-level sweep of individual attention heads in the 4 Transformer layers. The circuit is extracted by thresholding: nodes and edges with |recovery| above the threshold form the minimal subgraph. The hybrid architecture makes this particularly interesting — circuits may reveal how factual information encoded in Mamba's recurrent state gets read out by Transformer attention heads downstream.
